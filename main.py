...
class Node:
    def __init__(self,board, nodeDepth, heaurisitcValue, parentValue = None):
        self.board = board
    ...
    def create_possible_moves(self, parentValue = None):
        horizontalAxis, verticalAxis = self.find(self.board,'_')
        ...
        for element in allMoves:
            child = self.moveBlankSpot(self.board,horizontalAxis,verticalAxis,element[0],element[1])
    ...
    def moveBlankSpot(self,puz,horizontalAxis1,verticalAxis1,horizontalAxis2,verticalAxis2):
        if horizontalAxis2 >= 0 and horizontalAxis2 < len(self.board) and verticalAxis2 >= 0 and verticalAxis2 < len(self.board):
            puzzleInstance = []
    ...
    def find(self,puzzle,x):
        for element in range(0,len(self.board)):
            for secondElement in range(0,len(self.board)):
...
class App(tk.Frame):    
    ...
    def f(self,start,goal):
        return self.h(start.board,goal)+start.nodeDepth
    ...
    def process(self, n, start, goal):
        ...
        while True:
            ...
            closedList = []
            for i in closed:
                closedList.append(i.board)
            currentHasAnOpenNode = False
            for i in newOpen:
                if i.board not in closedList:
                    cur = i
